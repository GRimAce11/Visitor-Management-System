...
import { LoginDetails, VisitorDetails } from '../server/constant/Db';
import getAllData from '../server/Fetch/getAllData';
import Chart from 'react-google-charts';
import { MdDashboard, MdBrandingWatermark } from "react-icons/md";
...
const Admin = () => {
    ...
    const [FetchedCompaniesNames, setFetchedCompaniesNames] = useState([])

    const fetchData = async () => {
        const visitordata = await getAllData(VisitorDetails);
        const companydata = await getAllData(LoginDetails);

        console.log("Companies", companydata);
        // Fetch all Visitors
        const resultvisitor = visitordata.map((val) => [
            val.vname,
            val.vcompName,
            val.vdate,
            val.vtime,
        ...
        setFetchedVisitorData(() => resultvisitor);
        console.log("Visitors", FetchedVisitorData);

        // Fetch all Companies
        const resultcompany = companydata.map((val) => [
            val.companyname,
            val.username,
            val.password,
        ]);
        setFetchedCompanies(() => resultcompany);
        console.log("Companies list", FetchedCompanies);

        // Fetch only company names for select 
        const resultcompanyname = companydata.map((val) => [
            val.companyname
        ]);
        setFetchedCompaniesNames(() => resultcompanyname);
        console.log("Companies Name only list", FetchedCompaniesNames);

        // Fetch all Visitors Today
        const compdate = visitordata.filter((item) => item.vdate === todaytocheck);
        const resultvisitortoday = compdate.map((val) => [
            val.vname,
            val.vcompName,
            val.vdate,
            val.vtime,
        ...
        setFetchedVisitorDataToday(resultvisitortoday);
        console.log("Visitors for Today", FetchedVisitorDataToday);
    };
    useEffect(() => {
        fetchData();
    }, [username]);
    ...
    const carddata = [
        ...
        },
        {
            ...
            icon: <MdBrandingWatermark size={40} />,
            totalcount: FetchedCompanies.length,
        },
        {
            id: 3,
    ...
    return (
        <div className="min-h-screen flex flex-col flex-auto flex-shrink-0 antialiased bg-white dark:bg-gray-700 text-black ">
            ...
            <div className="fixed flex flex-col top-14 left-0 w-14 hover:w-64 md:w-64 bg-blue-900 dark:bg-gray-900 h-full text-white transition-all duration-300 border-none z-10 sidebar">
                <div className="overflow-y-auto overflow-x-hidden flex flex-col justify-between flex-grow">
                    <ul className="flex flex-col py-4 space-y-1">
                        ...
                        <li>
                            <Button onClick={showcompanydata} className={showcompanies ? 'relative flex flex-row items-center justify-start w-full h-11 focus:outline-none bg-blue-950  text-white-600 hover:text-white-800 border-l-4 border-transparent border-blue-500  pr-6' : 'relative flex flex-row items-center justify-start w-full h-11 focus:outline-none hover:bg-blue-800 dark:hover:bg-gray-600 text-white-600 hover:text-white-800 border-l-4 border-transparent hover:border-blue-500 dark:hover:border-gray-800 pr-6'}>
                                <MdBrandingWatermark size={25} />
                                <span className="ml-2 text-lg tracking-wide truncate">All Companies</span>
                            </Button>
                        </li>
            ...
            <div className="h-full ml-14 mt-14 mb-10 md:ml-64">
                {showdashboard &&
                    <main >
                        <div className="mb-4 pb-10 px-8 mx-4 rounded-3xl bg-gray-100 border-4 border-blue-400">
                ...
                <div className='flex justify-between px-10 w-full items-center'>
                    {/* <Button className=' bg-green-700 rounded-2xl text-white' onPress={modalcompany}>Add New Company</Button> */}
                    <Button className=' bg-blue-500 flex rounded-2xl text-white' onPress={onOpen}>Add <FaPlus /></Button>
                </div>
                            ...
                            <>
                                ...
                                <ModalBody>
                                    ...
                                    <Tabs defaultValue="Visitor" className="w-full">
                                        <TabsList className="w-full flex justify-between px-10">
                                            <TabsTrigger className="TabsTrigger" value="Visitor">Visitor</TabsTrigger>
                                            <TabsTrigger className="TabsTrigger" value="Company">Company</TabsTrigger>
                                        </TabsList>
                                            ...
                                            <form autoComplete="off">
                                                <div className="w-full flex flex-col gap-4">
                                                    ...
                                                    <div className="grid w-full items-center gap-1.5">
                                                        ...
                                                        />
                                                        {errors.vname &&
                                                            <span className='text-red-500 animate-pulse capitalize'>{errors.vname.message}</span>
                                                        }
                                                    </div>
                                                    ...
                                                    <div className="grid w-full items-center gap-1.5">
                                                        ...
                                                        >
                                                            {FetchedCompaniesNames.map((company) => (
                                                                <SelectItem key={company} className='bg-white text-black' value={company} aria-label={company}>
                                                                    {company}
                                                                </SelectItem>
                                                            ))}
                                                        </Select>
                                                        {errors.vcompany &&
                                                            <span className='text-red-500 animate-pulse capitalize'>{errors.vcompany.message}</span>
                                                        }
                                                    </div>
                                                    ...
                                                    <div className="grid w-full items-center gap-1.5">
                                                        ...
                                                        />
                                                        {errors.vdate &&
                                                            <span className='text-red-500 animate-pulse capitalize'>{errors.vdate.message}</span>
                                                        }
                                                    </div>
                                                    ...
                                                    <div className="grid w-full items-center gap-1.5">
                                                        ...
                                                        />
                                                        {errors.vtime &&
                                                            <span className='text-red-500 animate-pulse capitalize'>{errors.vtime.message}</span>
                                                        }
                                                    </div>
                                                    ...
                                                    <div className="grid w-full items-center gap-1.5">
                                                        ...
                                                        />
                                                        {errors.vmobile &&
                                                            <span className='text-red-500 animate-pulse capitalize'>{errors.vmobile.message}</span>
                                                        }
                                                    </div>
